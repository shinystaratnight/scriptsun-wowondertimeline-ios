...
class GetPostSticker :AddReactionDelegate,SharePostDelegate,comment_CountsDelegate{
    ...
    func getPostSticker (targetController :UIViewController,tableView : UITableView, indexpath:IndexPath, postFile : String, array : [[String:Any]],stackViewHeight: CGFloat,viewHeight: CGFloat, isHidden: Bool, viewColor :UIColor) -> UITableViewCell {
        ...
        for i in self.commentsIndex{
            if i["index"] as? Int == indexpath.row{
                cell.commentCountBtn.setTitle("\(NSLocalizedString("Comments", comment: "Comments"))\(" ")\(i["count"] ?? "")", for: .normal)
            }
        }
    ...
    func comment_Count() {
        let cell = self.tableView.cellForRow(at: IndexPath(row: self.selectedIndex, section: 6)) as! GifImageCell
            ...
            let total = count + 1
            cell.commentCountBtn.setTitle("\(NSLocalizedString("Comments", comment: "Comments"))\(" ")\(total)", for: .normal)
