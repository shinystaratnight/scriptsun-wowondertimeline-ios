...
class GeoCodeManager{
    ...
    func geoCode(type:String,lat:Double,lng:Double,completionBlock :@escaping (_ Success: GeoCodesModalMap.geoCode_SuccessModal?, _ AuthError: GeoCodesModalMap.geoCode_ErrorModal?, Error?)->()){
        ...
        Alamofire.request("https://maps.googleapis.com/maps/api/geocode/json?\(type)=\(lat),\(lng)&key=\(ControlSettings.googleApiKey)", method: .get, parameters: nil, encoding: URLEncoding.default, headers: nil).responseJSON { (response) in
